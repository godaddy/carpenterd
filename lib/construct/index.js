...
class Constructor extends EventEmitter {
  ...
  repack(spec, content, paths, statusWriter, next) {
    ...
    async.series({
      unpack: this.packer.perform('unpack', { content, installPath, statusWriter }),
      install: this.packer.perform('install', { spec, installPath, statusWriter }),
      pack: this.pack.bind(this, pkgDir, tarball, statusWriter),
      upload: this.packer.perform('upload', {spec, tarball, statusWriter})
    }, (err) => {
      if (err) return next(err);
