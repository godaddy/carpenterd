...
class Constructor extends EventEmitter {
  ...
  build({ promote, data }, done) {
    ...
    // MAKE ASYNC/AWAIT
    this.specs(data, async (error, spec) => {
      if (error) {
      ...
      let paths;
      try {
        paths = await this.prepare(spec, content, statusWriter);
      } catch (err) {
        return done(err);
      }
      ...
      const finish = async (err) => {
        ...
        const buildErr = await this.cleaner.cleanup(Object.keys(paths).map(key => paths[key]));
