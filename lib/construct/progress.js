...
Progress.prototype.write = function write(data, id, options = {}) {
  ...
  if (!options || !options.skipNsq) this._writeStatus(data);
 ...
 *
 * @param {Object} data Data for the event to write
 */
Progress.prototype._writeStatus = function writeStatus(data) {
  if (!data || !this.nsqStream || this.nsqStream._writableState.ended) {
    return;
  }

  const { name, env, version, locale, type: buildType } = this.metadata || {};
  const freshSpec = {
    eventType: 'event',
    name,
    env,
  ...
  this.nsqStream.write(freshSpec);
};
 ...
 */
Progress.prototype._endStatus = function endStatus(type, err) {
  if (!this.nsqStream || this.nsqStream._writableState.ended) {
    return;
  }
  ...
  const freshSpec = {
    ...
    version,
    buildType,
    total: Object.keys(this.map).length,
    message: err ? err.message : 'Builds Queued'
  };
  this.nsqStream.end(freshSpec);
};
...
Progress.prototype.end = function end(err) {
  ...
  this.stream.end();
  this._endStatus(err ? 'error' : 'queued', err);
