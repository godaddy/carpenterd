...
Constructor.prototype.buildPerLocale = function buildPerLocale(opts, next) {
  ...
  contructor.emit('queue', topic, freshSpec);
  return constructor.nsq.writer.publish(topic, freshSpec, (err) => {
    if (err) {
      app.contextLog.error('Build queue %s for %s env: %s failed %j', current.id, current.name, current.env, data);
      return step(err);
    }

    contructor.emit('queued', topic, freshSpec);
